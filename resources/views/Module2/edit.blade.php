...
<div class="max-w-4xl mx-auto py-10">
    ...
    <div class="bg-white shadow-2xl rounded-2xl p-10 border border-gray-100 w-full">
        ...
        <form action="{{ route('module2.update', $course->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="mb-6">
                ...
                <textarea name="description" rows="4"
                          class="w-full p-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 
                                 focus:border-blue-500 transition-all shadow-sm hover:shadow-md outline-none"
                          placeholder="Write a description about this course" required>{{ old('description', $course->description) }}</textarea>
            </div>
            ...
            <div class="mb-6">
                ...
                @if($course->image_url)
                    <img src="{{ asset('storage/'.$course->image_url) }}" class="mt-3 w-48 h-32 object-cover rounded">
                @endif
            </div>
            ...
            <div class="mb-8">
                ...
                <div class="flex items-center space-x-2">
                    @for ($i = 1; $i <= 5; $i++)
                        <svg xmlns="http://www.w3.org/2000/svg"
                             class="w-8 h-8 cursor-pointer {{ $i <= $course->difficulty ? 'text-yellow-400' : 'text-gray-300' }} transition-colors star"
                             data-value="{{ $i }}" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <input type="hidden" name="difficulty" id="difficulty" value="{{ old('difficulty', $course->difficulty) }}">
            ...
            <div class="mb-8">
                ...
                <input type="text" name="coordinator" value="{{ old('coordinator', $course->coordinator) }}"
                       class="w-full p-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 
                              focus:border-blue-500 transition-all shadow-sm hover:shadow-md outline-none"
