        ...
        <form action="{{ route('module2.update', $course->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-6">
                ...
                <textarea name="description" rows="4"
                          class="w-full p-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 
                                 focus:border-blue-500 transition-all shadow-sm hover:shadow-md outline-none"
                          placeholder="Write a description about this course" required>{{ $course->description }}</textarea>
            </div>
            ...
            <div class="mb-6">
                ...
                @if($course->image)
                    <div class="mb-3">
                        <img src="{{ asset('storage/'.$course->image) }}" alt="Course Image" class="w-48 h-32 object-cover rounded-md border">
                    </div>
                @endif
            ...
            <div class="mb-8">
                ...
                <div class="flex items-center space-x-2">
                    @for ($i = 1; $i <= 5; $i++)
                        <svg 
                            ...
                            stroke="currentColor"
                            class="w-8 h-8 cursor-pointer transition-colors star {{ $i <= $course->difficulty ? 'text-yellow-400' : 'text-gray-300' }}"
                            data-value="{{ $i }}"
                        >
                ...
                <input type="hidden" name="difficulty" id="difficulty" value="{{ $course->difficulty ?? 0 }}">
            ...
            <div class="mb-8">
                ...
                <input type="text" name="coordinator" value="{{ $course->coordinator }}"
                       class="w-full p-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 
                              focus:border-blue-500 transition-all shadow-sm hover:shadow-md outline-none"
